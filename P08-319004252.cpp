...
int main()
{
	...
	spotLightCount++;
	// Para la parte 2 del ejercicio generaremos 2 nuevos arreglos de luces puntutales
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// luz ligada a la cámara de tipo flash NO SE USA
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		BaseCoche.RenderModel();
		// Cofre carro
		// DADO QUE SE VA A ROTAR LAS COSAS POR EL ANGULO QUE TOME EL COFRE DEBEMO DE HEREDAR DESDE COFRE	
		model = modelaux;
		/*
		...
		*/
		...
		model = glm::scale(model, glm::vec3(.05f, .05f, .05f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		LLantaTraseraDereecha.RenderModel();
		...
		model = glm::scale(model, glm::vec3(.05f, .05f, .05f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		LLantaTraseraIzquerda.RenderModel();
